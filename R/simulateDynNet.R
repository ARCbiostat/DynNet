...
simulateDynNet <- function(Ni,
  ...
  if (is.null(structural.model$fixed.DeltaLP))
    stop("The argument structural.model$fixed.DeltaLP must be specified")
  ...
  if (is.null(parameters$Fixed.para.index))
    stop("The argument parameters$Fixed.para.index cannot be NULL")
  if (is.null(parameters$Fixed.para.values))
    stop("The argument parameters$Fixed.para.values cannot be NULL")
...
if (varcovRE.format == "cholesky"){
  nb_RE <- length(q0) + length(q)
  matD <-DparChol(nb_RE,unlist(parameters$paraB))
}
